...
TMatrixD CorrectCLAS_V(const TMatrixD &__VTrack,
  ...
  for(int i = 0; i < numParts; i++){
    ...
    if(__multi==false &&  __p_names[i]!="gamma" && __p_names[i]!="n" ) {
      if( __p_names[i]=="pi+" || __p_names[i]=="pi-") part_index = 1; // use pion parameters
      else if( __p_names[i]=="e+" || __p_names[i]=="e-") part_index = 2; // use pion parameters
      ...
      if(part_index == 2 && __isMC) p_res_scale *= 0.75;//here
      ...
      if(part_index == 2) V(3*i+1,3*i+1) *= 2.15*pow(p_res_scale,2);
      ...
      if(part_index == 2){
        if (__isMC) {
          p_offset = 0.5*(pPars[part_index][sector-1][bin][2]);
        }else{p_offset = 2.5*(pPars[part_index][sector-1][bin][2]);}//here
      }
      ...
      if(part_index == 2){
        V(3*i+2,3*i+2) *= 0.05*pow(res_scale,2);
        V(3*i+3,3*i+3) *= 0.05*pow(res_scale,2);
      }
